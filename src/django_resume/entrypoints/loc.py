...
else:  # pragma: no cover - used only to keep runtime imports optional
    Console = Any
...
def area_for_path(path: str | Path) -> str:
    ...
    if parts[0] == "src":
        return "src"
    if parts[0] == "docs" or path_obj.name == "README.rst":
        return "docs"
    if parts[0] == "example":
        return "example"
    if parts[0] == "talk":
...
def directory_bucket_for_path(path: str | Path) -> str:
    ...
    if parts[0] == "e2e_tests":
        return "e2e_tests"
    if parts[0] == "docs":
        return f"docs/{parts[1]}" if len(parts) >= 3 else "docs"
    if parts[0] == "example":
        return f"example/{parts[1]}" if len(parts) >= 3 else "example"
    if parts[0] == "talk":
        return f"talk/{parts[1]}" if len(parts) >= 3 else "talk"
